 ...
 */
export const validateCompactAddress = (
  address: string,
  ...
  return Result.tryCatch(
    () => {
      ...
      if (!COMPACT_ADDRESS_REGEX.test(trimmed)) {
        throw new Error(
          "Address must be a valid hex string starting with 0x and 40 characters long",
        );
      }
  ...
  return Result.tryCatch(
    () => {
      ...
      const gasEstimate = BigInt(
        21000 + fullCalldata.length * 16 + parameters.length * 1000,
      );
    ...
    case CompactType.INT256:
      ...
      const hex =
        signedNum >= 0n
          ...
          : (BigInt("0x" + "f".repeat(64)) + signedNum + 1n)
              .toString(16)
              .slice(-64);
      return hex;
...
const decodeParameter = (type: CompactType, data: string): unknown => {
  ...
  switch (type) {
    ...
    case CompactType.INT256:
      ...
      return value > maxPositive
        ? value - BigInt("0x1" + "0".repeat(64))
        : value;
  ...
  return Result.tryCatch(
    () => {
      ...
      const standardMessage =
        knownErrors[errorCode] || "Unknown contract error";
  ...
  return Result.tryCatch(
    () => {
      ...
      if (parameters.length !== functionDef.parameters.length) {
        throw new Error(
          `Parameter count mismatch: expected ${functionDef.parameters.length}, got ${parameters.length}`,
        );
      }
      ...
      for (let i = 0; i < parameters.length; i++) {
        ...
        if (param.type !== expectedParam.type) {
          throw new Error(
            `Parameter ${i} type mismatch: expected ${expectedParam.type}, got ${param.type}`,
          );
        }

        if (param.name !== expectedParam.name) {
          throw new Error(
            `Parameter ${i} name mismatch: expected ${expectedParam.name}, got ${param.name}`,
          );
        }
      }
    ...
    functions: [
      ...
      },
      {
        ...
        parameters: [
          {
            name: "proof",
            type: CompactType.BYTES,
            value: undefined,
            isPrivate: true,
          },
          {
            name: "nullifiers",
            type: CompactType.ARRAY,
            value: undefined,
            isPrivate: true,
          },
...
export const CompactUtils = {
  ...
  isPrivacyFunction: (func: CompactFunction): boolean => {
    return (
      func.parameters.some((p) => p.isPrivate) || func.visibility === "private"
    );
  },
  ...
  validateParameterValue: (
    param: CompactParameter,
  ): Result<boolean, CompactError> => {
    return Result.tryCatch(
      () => {
        if (param.value === undefined || param.value === null) {
          throw new Error(
            `Parameter ${param.name} cannot be null or undefined`,
          );
        }
