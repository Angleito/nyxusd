...
export declare const FP: {
  ...
  readonly replace: import("./curry").Curry3<
    string | RegExp,
    string,
    string,
    string
  >;
  readonly match: import("./curry").Curry2<
    RegExp,
    string,
  ...
  readonly memoize: <T extends any[], U>(
    fn: (...args: T) => U,
    keyFn?: (...args: T) => string,
  ) => (...args: T) => U;
  readonly debounce: <T extends any[]>(
    fn: (...args: T) => void,
    delay: number,
  ) => (...args: T) => void;
  readonly throttle: <T extends any[], U>(
    fn: (...args: T) => U,
    interval: number,
  ) => (...args: T) => U | undefined;
  readonly retry: <T extends any[], U>(
    fn: (...args: T) => U,
    maxAttempts: number,
    delay?: number,
  ) => (...args: T) => U;
