...
export declare abstract class Result<T, E = Error> {
     ...
     */
    static tryCatch<T, E = Error>(computation: () => T, errorMapper?: (error: unknown) => E): Result<T, E>;
    /**
     ...
     */
    static tryCatchAsync<T, E = Error>(computation: () => Promise<T>, errorMapper?: (error: unknown) => E): Promise<Result<T, E>>;
    /**
     ...
     */
    static fromPromise<T, E = Error>(promise: Promise<T>, errorMapper?: (error: unknown) => E): Promise<Result<T, E>>;
    /**
...
export declare const ResultUtils: {
     ...
     * @returns Function that works with Result values
     */
    lift2<A, B, C>(fn: (a: A, b: B) => C): <E>(resultA: Result<A, E>, resultB: Result<B, E>) => Result<C, E>;
    /**
     ...
     */
    all<T extends readonly Result<any, any>[]>(...results: T): Result<{ [K in keyof T]: T[K] extends Result<infer U, any> ? U : never; }, T[number] extends Result<any, infer E> ? E : never>;
};
