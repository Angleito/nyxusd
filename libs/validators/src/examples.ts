...
export const validateAddressExample = () => {
  ...
  console.log('Valid address result:', result1.success ? result1.data : result1.error.message);
  console.log('Invalid address result:', result2.success ? result2.data : result2.error.message);
};
...
export const validateAmountExample = () => {
  ...
  console.log('Invalid amount result:', result2.success ? result2.data : result2.error.message);
};
...
export const validateCDPCreationExample = () => {
  ...
  console.log('Invalid CDP creation:', result2.success ? 'Success' : `Error: ${result2.error.message}`);
};
...
export const chainValidationExample = () => {
  ...
  const checkBalance = (address: string) => {
    ...
    if (mockBalance > BigInt('500000000000000000000')) { // > 500 tokens
      return { success: true as const, data: { address, balance: mockBalance.toString() } };
    }
    return { 
  ...
  const result = chain(
    ...
    (validAddress) => checkBalance(validAddress)
  );
  
  console.log('Chained validation result:', result.success ? 'Valid with sufficient balance' : result.error.message);
};
...
export const combineValidationExample = () => {
  ...
  if (result.success) {
    ...
    console.log('All validations passed:', { validAddress, validAmount, validPrice });
  } else {
...
export const performanceExample = () => {
  ...
  console.log('Validation result:', result.success ? 'All valid' : 'Some failed');
};
