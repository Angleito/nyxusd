...
export const OracleFeedIdSchema = z.string()
  .min(1)
  .max(50)
  .regex(/^[A-Z]{2,10}-[A-Z]{2,10}$/, 'Feed ID must be in format ASSET-QUOTE (e.g., ETH-USD)');
...
// Oracle round ID
export const RoundIdSchema = z.bigint()
 ...
 */
export const OracleStatusSchema = z.enum(['healthy', 'degraded', 'critical', 'offline']);

export const OracleFeedHealthSchema = z.object({
 ...
 */
export const OracleResponseMetadataSchema = z.object({
...
export const ChainlinkRoundDataSchema = z.object({
  ...
  startedAt: z.bigint().refine(val => val >= 0n, 'StartedAt must be non-negative'),
  updatedAt: z.bigint().refine(val => val >= 0n, 'UpdatedAt must be non-negative'),
  answeredInRound: RoundIdSchema,
}).strict().refine(
  (data) => data.updatedAt >= data.startedAt,
  {
...
export const PriceValidationRulesSchema = z.object({
  minPrice: z.bigint().refine(val => val > 0n, 'MinPrice must be positive').optional(),
  maxPrice: z.bigint().refine(val => val > 0n, 'MaxPrice must be positive').optional(),
  maxDeviation: z.number().min(0).max(100).optional(),
  referencePrice: z.bigint().refine(val => val > 0n, 'ReferencePrice must be positive').optional(),
...
export const PriceValidationIssueSchema = z.object({
  code: z.string().min(1),
...
export const AggregationSourceSchema = z.object({
  provider: z.string().min(1).max(20),
  price: z.bigint().refine(val => val > 0n, 'Source price must be positive'),
...
export const AggregationStatisticsSchema = z.object({
...
export const AggregationOutlierSchema = z.object({
...
export const ConsensusInfoSchema = z.object({
...
export const AggregationResultSchema = z.object({
  aggregatedPrice: z.bigint().refine(val => val > 0n, 'Aggregated price must be positive'),
...
export const OracleNetworkConfigSchema = z.object({
  name: z.string().min(1),
...
export const OracleServiceConfigSchema = z.object({
 ...
 */
export const AIRiskFactorSchema = z.object({
