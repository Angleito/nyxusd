...
export const CollateralAssetConfigSchema = z.object({
  ...
  symbol: z.string().min(1, 'Symbol cannot be empty').max(10, 'Symbol too long'),
  ...
  priceValidityPeriod: z.number().int().positive('Price validity period must be positive'),
...
export const CollateralDepositSchema = z.object({
  ...
  minHealthFactor: z.number().positive('Minimum health factor must be positive').default(1.1),
...
export const CollateralWithdrawalSchema = z.object({
  ...
  minHealthFactorAfterWithdrawal: z.number().positive('Minimum health factor must be positive').default(1.2),
...
export const CollateralRiskAssessmentSchema = z.object({
  ...
  externalRatings: z.array(z.object({
    provider: z.string(),
    rating: z.string(),
