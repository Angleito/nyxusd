...
const price = PriceSchema.parse(1234.56789); // Rounded to 6 decimals
...
const result: Result<Address, ValidationError> = validate(
  AddressSchema,
  userInput,
);
...
// Chain validations
const result = chain(validate(AddressSchema, userAddress), (address) =>
  validateUserPermissions(address),
);

// Map over successful results
const upperCaseResult = map(validate(z.string(), userInput), (str) =>
  str.toUpperCase(),
);
