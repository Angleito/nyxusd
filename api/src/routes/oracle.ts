    ...
    try {
      ...
      const oracleConfig = {
        primaryOracle: {
          ...
          defaultMaxStaleness: parseInt(process.env.ORACLE_MAX_STALENESS || '3600', 10),
          defaultMinConfidence: parseFloat(process.env.ORACLE_MIN_CONFIDENCE || '95'),
...
router.get('/prices', async (req: Request, res: Response) => {
  try {
    ...
    const requestedFeeds = feeds 
      ...
      : ['ETH-USD', 'BTC-USD', 'LINK-USD', 'UNI-USD', 'USDC-USD', 'USDT-USD', 'DAI-USD'];
    ...
    const pricePromises = requestedFeeds.map(async (feedId) => {
      try {
        ...
        if (result._tag === 'Right') {
          ...
          const formattedPrice = Number(priceData.price) / Math.pow(10, priceData.decimals);
    ...
    if (format === 'detailed') {
      ...
      failedResults.forEach(result => {
        const displayPair = result.feedId.replace('-', '/');
        const fallbackPrice = FALLBACK_PRICES[displayPair as keyof typeof FALLBACK_PRICES];
        if (fallbackPrice) {
      ...
      res.json({
        ...
        errors: failedResults.length > 0 ? failedResults.map(r => ({
          feed: r.feedId,
          error: r.error,
        })) : undefined,
      });
