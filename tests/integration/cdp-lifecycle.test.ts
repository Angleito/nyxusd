...
describe('CDP Lifecycle Integration Tests', () => {
  describe('CDP Creation and Management', () => {
    test('should create CDP with valid collateral', async () => {
      ...
      const createCDP = async (request: typeof createRequest) => {
        ...
        const collateralValue = Number(request.initialCollateral.amount) / Math.pow(10, collateralAsset.decimals);
    ...
    test('should reject CDP creation with insufficient collateral', async () => {
      ...
      const createCDP = async (request: typeof createRequest) => {
        const collateralValue = Number(request.initialCollateral.amount) / Math.pow(10, 18);
        const minValue = 1000; // $1000 minimum
  ...
  describe('Collateral Operations', () => {
    test('should process collateral deposit successfully', async () => {
      ...
      const processDeposit = async (cdp: typeof baseCDP, operation: typeof depositOperation) => {
        if (operation.operation !== 'deposit') {
          return { success: false, error: 'Invalid operation type' };
        }
        
        // Calculate new collateral value
        const additionalValue = Number(operation.amount) / Math.pow(10, 18) * 1600; // ETH price
        const newCollateralValue = cdp.collateralValue + additionalValue;
        
        // Calculate new health factor
        const newHealthFactor = newCollateralValue / Number(cdp.debtAmount) * 100;
      ...
      expect(result.success).toBe(true);
      expect(result.cdp?.collateralValue).toBeGreaterThan(baseCDP.collateralValue);
    ...
    test('should prevent withdrawal that would cause liquidation', async () => {
      ...
      const processWithdrawal = async (cdp: typeof riskyCDP, operation: typeof withdrawalOperation) => {
        if (operation.operation !== 'withdraw') {
          return { success: false, error: 'Invalid operation type' };
        }
        
        // Calculate remaining collateral value
        const withdrawalValue = Number(operation.amount) / Math.pow(10, 8) * 42000; // BTC price
        const remainingCollateralValue = cdp.collateralValue - withdrawalValue;
        
        // Calculate resulting health factor
        const newHealthFactor = remainingCollateralValue / Number(cdp.debtAmount) * 100;
        
        // Check minimum health factor requirement
        if (newHealthFactor < operation.minHealthFactorAfterWithdrawal) {
          return { success: false, error: 'Withdrawal would violate minimum health factor' };
        }
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Withdrawal would violate minimum health factor');
    });
  });
  ...
  describe('Debt Operations', () => {
    test('should process debt minting with collateral check', async () => {
      ...
      const processMint = async (cdp: typeof baseCDP, operation: typeof mintOperation) => {
        ...
        const newCollateralizationRatio = (cdp.collateralValue / Number(newDebtAmount)) * 100;
        
        // Check minimum collateralization ratio
        if (newCollateralizationRatio < operation.minCollateralizationRatioAfter / 100) {
          return { success: false, error: 'Minting would violate minimum collateralization ratio' };
        }
        ...
        if (operation.includeAccruedFees) {
          ...
          fees = BigInt(Math.floor(Number(newDebtAmount) * (cdp.stabilityFee / 10000) * (timeElapsed / (365 * 24 * 3600))));
          
          if (operation.maxFeeAmount && fees > operation.maxFeeAmount) {
            return { success: false, error: 'Fee amount exceeds maximum allowed' };
          }
        }
      ...
      expect(result.cdp?.collateralizationRatio).toBeGreaterThan(operation.minCollateralizationRatioAfter / 100);
    });
    ...
    test('should process debt repayment', async () => {
      ...
      const processRepay = async (cdp: typeof baseCDP, operation: typeof repayOperation) => {
        ...
        if (operation.amount > cdp.debtAmount) {
          return { success: false, error: 'Repayment amount exceeds total debt' };
        }
        ...
        const newCollateralizationRatio = newDebtAmount > 0n 
        ...
        const updatedCDP = {
          ...
          debtAmount: newDebtAmount,
          collateralizationRatio: newCollateralizationRatio,
          healthFactor: newCollateralizationRatio === Infinity ? Infinity : newCollateralizationRatio / 100,
          lastUpdated: Math.floor(Date.now() / 1000),
        };
  ...
  describe('Liquidation Process', () => {
    test('should execute liquidation for unhealthy CDP', async () => {
      ...
      const processLiquidation = async (cdp: typeof unhealthyCDP, liquidator: string) => {
        ...
        if (cdp.collateralValue < totalDebtValue + liquidationPenalty) {
          return { success: false, error: 'Insufficient collateral for liquidation' };
        }
        ...
        const liquidationResult = {
          ...
          remainingCollateral: BigInt(Math.floor(cdp.collateralValue - totalDebtValue - liquidationPenalty)),
          remainingDebt: 0n,
      ...
      const result = await processLiquidation(unhealthyCDP, liquidationData.liquidator);
      
      expect(result.success).toBe(true);
      expect(result.liquidation?.triggerReason).toBe('health_factor_below_threshold');
    ...
    test('should prevent liquidation of healthy CDP', async () => {
      ...
      const processLiquidation = async (cdp: typeof healthyCDP, liquidator: string) => {
        if (cdp.healthFactor >= 1.0) {
          return { success: false, error: 'CDP is not liquidatable' };
        }
      ...
      const result = await processLiquidation(healthyCDP, '0x1234567890123456789012345678901234567890');
  ...
  describe('Price Oracle Integration', () => {
    test('should handle price updates and recalculate health factors', async () => {
      ...
      const handlePriceUpdate = async (cdp: typeof cdp, priceUpdate: typeof priceUpdate) => {
        ...
        const priceChangeRatio = priceUpdate.newPrice / (priceUpdate.previousPrice || priceUpdate.newPrice);
    ...
    test('should handle oracle failures gracefully', async () => {
      ...
      const handlePriceUpdate = async (cdp: typeof cdp, priceData: typeof failedPriceUpdate) => {
      ...
      expect(result.staleness).toBeGreaterThan(failedPriceUpdate.stalenessThreshold);
    });
  });
    ...
    test('should manage diversified collateral portfolio', async () => {
      ...
      const checkRebalancingNeed = (portfolio: typeof portfolio) => {
        const needsRebalancing = portfolio.collateralAssets.some(asset => {
          const deviation = Math.abs(asset.weight - (asset.allocationTarget || asset.weight));
        ...
        return {
          ...
          currentAllocations: portfolio.collateralAssets.map(asset => ({
            ...
            deviation: Math.abs(asset.weight - (asset.allocationTarget || asset.weight)),
          })),
        };
      };
      ...
      expect(result.currentAllocations).toHaveLength(portfolio.collateralAssets.length);
      expect(result.currentAllocations.every(a => a.deviation >= 0)).toBe(true);
    });
  });
  ...
  describe('Stress Testing Scenarios', () => {
    test('should handle market crash scenario', async () => {
      ...
      const simulateMarketCrash = async (cdps: typeof initialCDPs, scenario: typeof crashScenario) => {
        const results = cdps.map(cdp => {
          ...
          return {
            ...
            newHealthFactor,
            isLiquidated,
            newRiskLevel: isLiquidated ? 'critical' : 
                         newHealthFactor < 1.2 ? 'high' :
                         newHealthFactor < 1.5 ? 'medium' : 'low',
          };
        });
        
        const liquidatedCount = results.filter(r => r.isLiquidated).length;
        const totalAtRisk = results.filter(r => r.newHealthFactor < 1.2).length;
