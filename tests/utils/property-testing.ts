...
export const FinancialProperties = {
   ...
   */
  nonNegativeCollateral:
    <T>(gen: fc.Arbitrary<T>, getCollateralValue: (input: T) => number) =>
    (config?: PropertyTestConfig) => {
      const testConfig = { ...PROPERTY_TEST_CONFIG, ...config };
   ...
   */
  collateralizationRatioPreservation:
    <T>(
      gen: fc.Arbitrary<T>,
      operation: (input: T) => T,
    ...
    (config?: PropertyTestConfig) => {
      ...
      fc.assert(
        fc.property(genT, genE, (value, error) => {
          ...
          return (
            okLeft.toString() === okResult.toString() &&
      ...
      fc.assert(
        fc.property(genT, genE, (value, error) => {
          ...
          return (
            okLeft.toString() === okRight.toString() &&
    ...
    (config?: PropertyTestConfig) => {
      ...
      fc.assert(
        fc.property(genT, genE, (value, error1) => {
          ...
          return (
            chainedResult.isErr() && (chainedResult as any).value === error1
          );
        }),
        testConfig,
      );
...
export const CDPProperties = {
   ...
   */
  cdpInvariants:
    <T>(gen: fc.Arbitrary<T>, getCDP: (input: T) => any) =>
    (config?: PropertyTestConfig) => {
      const testConfig = { ...PROPERTY_TEST_CONFIG, ...config };
