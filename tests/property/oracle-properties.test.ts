...
describe('Oracle Service Properties', () => {
        ...
        async (feedId, iterations) => {
          ...
          if (prices.length >= 2) {
            ...
            if (minPrice > BigInt(0)) {
              const variation = Number((maxPrice - minPrice) * BigInt(100) / minPrice);
  ...
  describe('Privacy Oracle Properties', () => {
    ...
    it('should verify generated proofs successfully', () => {
      fc.assert(fc.asyncProperty(
        ...
        async (price, nonce) => {
          ...
          const generateResult = await privacyService.fetchPrivatePrice(query);
    ...
    it('should generate deterministic commitments for same inputs', () => {
      fc.assert(fc.asyncProperty(
        ...
        fc.hexaString({ minLength: 16, maxLength: 32 }),
        async (price, nonce) => {
          const commitment1 = await privacyService.generatePriceCommitment(price, nonce);
          const commitment2 = await privacyService.generatePriceCommitment(price, nonce);
      ...
      fc.assert(fc.asyncProperty(
        ...
        async () => {
          ...
          if (isRight(result)) {
            ...
            expect(health.metrics.healthyFeeds).toBeLessThanOrEqual(health.metrics.totalFeeds);
            expect(health.metrics.averageConfidence).toBeGreaterThanOrEqual(0);
  ...
  describe('Error Handling Properties', () => {
    it('should handle invalid feed IDs gracefully', () => {
      fc.assert(fc.asyncProperty(
        fc.string().filter(s => !['ETH/USD', 'BTC/USD', 'ADA/USD', 'DUST/USD'].includes(s)),
        async (invalidFeedId) => {
          const query = {
      ...
      fc.assert(fc.asyncProperty(
        ...
        async (feedId, minConfidence) => {
          ...
          if (isRight(result)) {
            ...
            expect(result.right.data.confidence).toBeGreaterThanOrEqual(minConfidence);
          }
  ...
  describe('Price Aggregation Mathematics', () => {
    it('should satisfy median properties', () => {
      fc.assert(fc.property(
        fc.array(fc.bigInt({ min: 1000000n, max: 10000000000n }), { minLength: 3, maxLength: 10 }),
        (prices) => {
          ...
          if (sortedPrices.length % 2 === 1) {
            expect(Math.abs(smallerCount - largerCount)).toBeLessThanOrEqual(1);
          }
    ...
    it('should satisfy weighted average properties', () => {
      fc.assert(fc.property(
        ...
        (priceWeights) => {
          const totalWeight = priceWeights.reduce((sum, pw) => sum + pw.weight, 0);
          ...
          const minPrice = Math.min(...priceWeights.map(pw => Number(pw.price)));
          const maxPrice = Math.max(...priceWeights.map(pw => Number(pw.price)));
    ...
    it('should detect outliers correctly using z-score', () => {
      fc.assert(fc.property(
        fc.array(fc.float({ min: 1000, max: 10000 }), { minLength: 5, maxLength: 20 }),
  ...
  describe('Confidence Score Mathematics', () => {
    it('should maintain confidence bounds', () => {
      fc.assert(fc.property(
        ...
        fc.array(fc.float({ min: 0.1, max: 5 }), { minLength: 1, maxLength: 10 }),
        (confidences, weights) => {
          ...
          const weightedConfidence = confSlice.reduce(
            (sum, c, i) => sum + c * weightSlice[i],
            0
          ) / totalWeight;
