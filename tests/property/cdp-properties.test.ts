...
describe("CDP Mathematical Properties", () => {
  describe("Health Factor Properties", () => {
    ...
    PropertyTesting.run("Health factor calculation is consistent", () => {
    ...
    PropertyTesting.run("Liquidation triggers when health factor < 1", () => {
  ...
  describe("Collateralization Ratio Properties", () => {
    PropertyTesting.run(
      ...
      () => {
        PropertyTesting.math.monotonicity(
          ...
          (a, b) =>
            a.collateralValue <= b.collateralValue &&
    ...
    PropertyTesting.run("Adding collateral improves health factor", () => {
      fc.assert(
        fc.property(
          DataGenerators.cdp.cdpConfig(),
          ...
          (baseCDP, additionalCollateral) => {
            ...
            const newCollateralValue =
              originalCollateralValue + additionalValue;
            const newHealthFactor =
              (newCollateralValue * (baseCDP.collateralizationRatio / 100)) /
              Number(baseCDP.debtAmount);
  ...
  describe("Fee Accrual Properties", () => {
    PropertyTesting.run("Fees accrue monotonically over time", () => {
  ...
  describe("Risk Level Properties", () => {
    PropertyTesting.run("Risk level correlates with health factor", () => {
      fc.assert(
        fc.property(DataGenerators.cdp.cdpConfig(), (cdp) => {
      ...
      () => {
        fc.assert(
          fc.property(
            DataGenerators.cdp
              .cdpConfig()
              .filter((cdp) => cdp.riskLevel === "critical"),
            (cdp) => {
  ...
  describe("Liquidation Properties", () => {
    PropertyTesting.run("Liquidation price calculation is consistent", () => {
      fc.assert(
        fc.property(DataGenerators.cdp.cdpConfig(), (cdp) => {
          ...
          const expectedLiquidationPrice =
            Number(debtAmount) / (impliedCollateralAmount * 0.8); // Assuming 80% LTV

          const tolerance = expectedLiquidationPrice * 0.1; // 10% tolerance
          return (
            Math.abs(liquidationPrice - expectedLiquidationPrice) <= tolerance
          );
        }),
      );
    });
    ...
    PropertyTesting.run(
      ...
      () => {
        fc.assert(
          fc.property(
            CDPFixtures.samples.liquidations[0]
              ? fc.constant(CDPFixtures.samples.liquidations[0])
              : fc.constant(null),
            (liquidation) => {
              ...
              const totalValueAfterLiquidation =
                totalDebtAmount + liquidationPenalty + liquidatorReward;
  ...
  describe("Portfolio-Level Properties", () => {
    PropertyTesting.run(
      ...
      () => {
        fc.assert(
          fc.property(
            fc.array(DataGenerators.cdp.cdpConfig(), {
              minLength: 2,
              maxLength: 5,
            }),
            (cdps) => {
              const totalCollateralValue = cdps.reduce(
                (sum, cdp) => sum + cdp.collateralValue,
                0,
              );
              const totalDebtValue = cdps.reduce(
                (sum, cdp) => sum + Number(cdp.debtAmount),
                0,
              );
              ...
              return (
                Math.abs(portfolioHealthFactor - weightedAverage) <= tolerance
              );
            },
          ),
        );
    ...
    PropertyTesting.run("Diversification reduces portfolio risk", () => {
      fc.assert(
        fc.property(
          fc.tuple(
  ...
  describe("Mathematical Invariants", () => {
    PropertyTesting.run("Addition is commutative for collateral values", () => {
    ...
    PropertyTesting.run("Zero is identity for addition", () => {
    ...
    PropertyTesting.run("One is identity for multiplication", () => {
      PropertyTesting.math.identity(
        fc.float({ min: 0.01, max: 100 }),
        (a, identity) => a * identity,
        1,
  ...
  describe("Edge Case Properties", () => {
    PropertyTesting.run("Minimal CDPs maintain basic invariants", () => {
      fc.assert(
        fc.property(fc.constant(CDPFixtures.edgeCases.minimalCDP), (cdp) => {
          return (
    ...
    PropertyTesting.run("Maximal CDPs maintain basic invariants", () => {
      fc.assert(
        fc.property(fc.constant(CDPFixtures.edgeCases.maximalCDP), (cdp) => {
          return (
            cdp.collateralValue >= 0 &&
            cdp.debtAmount >= 0n &&
            cdp.healthFactor >= 0 &&
    ...
    PropertyTesting.run(
      ...
      () => {
        fc.assert(
          fc.property(fc.constant(CDPFixtures.edgeCases.zeroDebtCDP), (cdp) => {
            return (
              cdp.debtAmount === 0n &&
              (cdp.healthFactor === Infinity || cdp.healthFactor > 100)
            );
          }),
        );
      },
...
describe("CDP State Transition Properties", () => {
  PropertyTesting.run("CDP status transitions are valid", () => {
    fc.assert(
      fc.property(
        fc.tuple(
  ...
  PropertyTesting.run("Collateral operations preserve total supply", () => {
    fc.assert(
      fc.property(
        DataGenerators.financial.collateralAmount(),
        DataGenerators.financial.collateralAmount(),
...
describe("CDP Validation Properties", () => {
  PropertyTesting.run("Valid CDPs pass all validation rules", () => {
    fc.assert(
      fc.property(DataGenerators.cdp.cdpConfig(), (cdp) => {
        // Basic validation rules
        const hasValidId = cdp.id.length > 0;
        const hasValidOwner =
          cdp.owner.startsWith("0x") && cdp.owner.length === 42;
        ...
        const hasValidAmounts =
          cdp.collateralValue >= 0 && cdp.debtAmount >= 0n;
  ...
  PropertyTesting.run("Invalid CDPs fail validation", () => {
    fc.assert(
      fc.property(
        ...
        (invalidAddress) => {
          ...
          const isValid =
            invalidAddress.startsWith("0x") && invalidAddress.length === 42;
