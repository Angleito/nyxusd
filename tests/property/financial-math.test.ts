...
describe("Financial Mathematics Properties", () => {
  describe("Interest Rate Calculations", () => {
    ...
    PropertyTesting.run("Simple interest is linear with time", () => {
      PropertyTesting.math.monotonicity(
        fc.record({
          principal: fc.float({ min: 1000, max: 100000 }),
          rate: fc.float({ min: 0.01, max: 0.2 }),
          time: fc.float({ min: 0, max: 10 }),
        }),
        (params) =>
          calculateSimpleInterest(params.principal, params.rate, params.time),
        (a, b) =>
          a.principal === b.principal && a.rate === b.rate && a.time <= b.time,
        (a, b) => a <= b,
      )();
    });

    PropertyTesting.run("Compound interest grows exponentially", () => {
      fc.assert(
        fc.property(
          ...
          (principal, rate, time) => {
            ...
            const simpleInterest =
              calculateSimpleInterest(principal, rate, time) + principal;
    ...
    PropertyTesting.run("Zero interest rate yields zero interest", () => {
      fc.assert(
        fc.property(
          fc.float({ min: 1000, max: 100000 }),
        ...
        fc.assert(
          fc.property(
            ...
            (value, rate, time) => {
              ...
              const backToPresentValue = calculatePresentValue(
                futureValue,
                rate,
                time,
              );
      ...
      () => {
        PropertyTesting.math.monotonicity(
          fc.record({
            ...
            discountRate: fc.float({ min: 0.01, max: 0.5 }),
  ...
  describe("Risk Metrics", () => {
    ...
    PropertyTesting.run(
      ...
      () => {
        PropertyTesting.math.monotonicity(
          fc.record({
            returns: fc.float({ min: 0.01, max: 0.5 }),
            riskFreeRate: fc.float({ min: 0.001, max: 0.05 }),
            volatility: fc.float({ min: 0.01, max: 0.3 }),
          }),
          (params) =>
    ...
    PropertyTesting.run(
      ...
      () => {
        fc.assert(
          fc.property(
            fc.float({ min: 0.05, max: 0.2 }),
            fc.tuple(
              fc.float({ min: 0.1, max: 0.5 }),
              fc.float({ min: 0.51, max: 1.0 }),
            ),
    ...
    PropertyTesting.run(
      ...
      () => {
        PropertyTesting.math.monotonicity(
          ...
          (a, b) =>
            a.maxLTV === b.maxLTV && a.collateralValue <= b.collateralValue,
          (a, b) => a <= b,
        )();
      },
    );

    PropertyTesting.run("LTV calculation is commutative for scaling", () => {
      fc.assert(
        fc.property(
          fc.float({ min: 1000, max: 100000 }),
          ...
          (collateralValue, loanAmount, scale) => {
            ...
            const ltv2 = calculateLTV(
              loanAmount * scale,
              collateralValue * scale,
            );
  ...
  describe("Liquidation Calculations", () => {
     ...
     */
    const calculateLiquidationPenalty = (
      ...
      () => {
        PropertyTesting.math.monotonicity(
          fc.record({
            ...
            penaltyRate: fc.float({ min: 0.05, max: 0.2 }),
          }),
          (params) =>
            calculateLiquidationPenalty(
              params.liquidatedAmount,
              params.penaltyRate,
            ),
          (a, b) =>
            a.penaltyRate === b.penaltyRate &&
            a.liquidatedAmount <= b.liquidatedAmount,
          (a, b) => a <= b,
        )();
      },
    );
        ...
        fc.assert(
          fc.property(
            ...
            (collateralValue, debtValue, liquidationThreshold) => {
              const shouldLiquidate = isLiquidatable(
                collateralValue,
                debtValue,
    ...
    PropertyTesting.run("Zero penalty rate results in zero penalty", () => {
      fc.assert(
        fc.property(
          fc.float({ min: 1000, max: 100000 }),
          (liquidatedAmount) => {
  ...
  describe("Portfolio Calculations", () => {
    ...
    const calculatePortfolioValue = (
      assets: Array<{ amount: number; price: number }>,
    ): number => {
    ...
    PropertyTesting.run("Portfolio weights sum to 1", () => {
      fc.assert(
        fc.property(
          fc.array(
            fc.record({
          ...
          (assets) => {
            ...
            const weights = assets.map((asset) =>
              calculateAssetWeight(asset.amount * asset.price, portfolioValue),
            );
  ...
  describe("Price Impact Models", () => {
     ...
     */
    const calculateLinearPriceImpact = (
      tradeSize: number,
      liquidity: number,
    ): number => {
     ...
     */
    const calculateSqrtPriceImpact = (
      tradeSize: number,
      liquidity: number,
    ): number => {
      if (liquidity === 0) return Infinity;
    ...
    PropertyTesting.run(
      ...
      () => {
        PropertyTesting.math.monotonicity(
          ...
          (params) =>
            calculateLinearPriceImpact(params.tradeSize, params.liquidity),
        ...
        fc.assert(
          fc.property(
            ...
            (tradeSize, liquidity) => {
              ...
              const linearImpact = calculateLinearPriceImpact(
                tradeSize,
                liquidity,
              );
    ...
    PropertyTesting.run("Zero trade size results in zero price impact", () => {
      fc.assert(
        fc.property(fc.float({ min: 1000, max: 1000000 }), (liquidity) => {
