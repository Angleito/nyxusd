 ...
 * ```
 */
export const validateCDPCreation = (
  params: CDPCreationParams,
  context: CDPCreationContext,
  ...
  const collateralValue =
    (params.collateralAmount * context.collateralPrice) / BigInt(10 ** 18); // Assuming 18 decimal scaling
  const collateralizationRatio = Number(
    (collateralValue * BigInt(10000)) / params.debtAmount,
  ); // In basis points
  ...
  if (collateralizationRatio < context.config.minCollateralizationRatio) {
    return Result.err({
      ...
      minimum: mkCollateralizationRatio(
        context.config.minCollateralizationRatio,
      ),
    });
  }
...
export const createCDP = (
  ...
  const healthFactor = calculateHealthFactor(
    ...
    params.config.liquidationRatio,
  );

  // Create initial state
  const initialState = createInitialCDPState(
    healthFactor,
    params.config.liquidationRatio,
  );
 ...
 * ```
 */
export const createCDPBatch = (
  requests: readonly CDPCreationParams[],
  context: CDPCreationContext,
): Result<readonly CDP[], CDPError> => {
  const cdps: CDP[] = [];
...
export const estimateMaxDebt = (
  ...
  const collateralValue =
    (collateralAmount * collateralPrice) / BigInt(10 ** 18);
  const maxDebt =
    (collateralValue * BigInt(10000)) / BigInt(minCollateralizationRatio);
 ...
 * ```
 */
export const estimateMinCollateral = (
  debtAmount: Amount,
  collateralPrice: bigint,
  minCollateralizationRatio: number,
): Amount => {
  const requiredCollateralValue =
    (debtAmount * BigInt(minCollateralizationRatio)) / BigInt(10000);
  const minCollateral =
