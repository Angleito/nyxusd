...
export type LiquidationValidationError = 
  | { readonly type: 'position_not_liquidatable'; readonly currentRatio: number; readonly liquidationRatio: number }
  | { readonly type: 'invalid_liquidation_amount'; readonly amount: bigint; readonly reason: string }
  | { readonly type: 'exceeds_maximum_liquidation'; readonly maximum: Amount; readonly requested: Amount }
  | { readonly type: 'liquidator_insufficient_balance'; readonly required: Amount; readonly available: Amount }
  | { readonly type: 'invalid_liquidator'; readonly address: string; readonly reason: string }
  | { readonly type: 'liquidation_amount_too_small'; readonly minimum: Amount; readonly provided: Amount }
...
export const validateLiquidationAmount = (
  ...
  try {
    ...
    const maxAllowedByPercentage = (debtAmount as bigint) * BigInt(MAX_LIQUIDATION_PERCENTAGE) / 10000n
...
export const isValidLiquidator = (
  ...
  if (!addressPattern.test(liquidatorAddress)) {
    return Result.err({
      type: 'invalid_liquidator',
      address: liquidatorAddress,
      reason: 'Invalid address format. Expected 42-character hex string starting with 0x'
    })
  }
...
export const calculateLiquidationBonus = (
  ...
  if (bonusPercentage > 2000) { // Max 20% bonus
    return Result.err({
      type: 'invalid_ratio',
      ratio: bonusPercentage,
  ...
  try {
    ...
    const bonus = (liquidationAmount as bigint) * BigInt(bonusPercentage) / 10000n
...
export const validateLiquidation = (
  ...
  const liquidatableCheck = isLiquidatable(collateralValue, debtAmount, liquidationRatio)
  if (liquidatableCheck.isErr()) {
    return liquidatableCheck
  }

  // Validate liquidation amount
  const amountCheck = validateLiquidationAmount(debtAmount, liquidationAmount, maxLiquidation)
  if (amountCheck.isErr()) {
    return amountCheck
  }

  // Validate liquidator
  const liquidatorCheck = isValidLiquidator(liquidatorAddress, minLiquidatorBalance, liquidatorBalance)
 ...
 */
export const composeLiquidationValidations = <T>(
  validations: Array<(input: T) => Result<boolean, LiquidationValidationError>>
