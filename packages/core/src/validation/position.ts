...
export type PositionValidationError =
  | {
      readonly type: "position_unhealthy";
      readonly currentRatio: number;
      readonly minimumRatio: number;
    }
  | {
      readonly type: "position_undercollateralized";
      readonly currentRatio: number;
      readonly liquidationRatio: number;
    }
  | {
      readonly type: "insufficient_collateral_for_mint";
      readonly requiredCollateral: Amount;
      readonly availableCollateral: Amount;
    }
  | {
      readonly type: "insufficient_collateral_for_withdrawal";
      readonly requiredCollateral: Amount;
      readonly availableAfterWithdrawal: Amount;
    }
  | {
...
export const canWithdrawCollateral = (
  ...
  try {
    ...
    const remainingCollateral =
      (collateralValue as bigint) - (withdrawAmount as bigint);

    // Calculate required collateral for current debt
    const requiredCollateral =
      ((debtAmount as bigint) * BigInt(minRatio)) / 10000n;
...
export const validateStateTransition = (
  ...
  if (targetState === "liquidating") {
    ...
    const undercollateralizedCheck = isPositionUndercollateralized(
      collateralValue,
      debtAmount,
      minRatio,
    );
 ...
 */
export const composePositionValidations =
  <T>(
