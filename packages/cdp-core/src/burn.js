 ...
 * ```
 */
export const calculateAccruedFees = (currentDebt, stabilityFeeRate, timeElapsed) => {
 ...
 * ```
 */
export const calculateBurnAllocation = (burnAmount, currentDebt, accruedFees) => {
 ...
 * ```
 */
export const validateBurnNYXUSD = (params, context) => {
    ...
    const accruedFees = calculateAccruedFees(params.cdp.debtAmount, context.stabilityFeeRate, context.timeElapsed);
 ...
 * ```
 */
export const calculateHealthFactorAfterBurn = (collateralAmount, currentDebt, principalRepaid, collateralPrice, liquidationRatio) => {
 ...
 * ```
 */
export const calculateCurrentHealthFactor = (cdp, collateralPrice) => {
 ...
 * ```
 */
export const updateCDPStateAfterBurn = (currentState, remainingDebt, newHealthFactor, autoClose, timestamp) => {
 ...
 * ```
 */
export const createUpdatedCDP = (cdp, principalRepaid, feesPaid, newState, timestamp) => {
    return {
        ...cdp,
 ...
 * ```
 */
export const burnNYXUSD = (params, context) => {
    ...
    const allocation = calculateBurnAllocation(params.burnAmount, params.cdp.debtAmount, accruedFees);
    ...
    const newHealthFactor = calculateHealthFactorAfterBurn(params.cdp.collateralAmount, params.cdp.debtAmount, allocation.principalPayment, context.collateralPrice, params.cdp.config.liquidationRatio);
    // Update CDP state based on remaining debt and system settings
    const newState = updateCDPStateAfterBurn(params.cdp.state, allocation.remainingDebt, newHealthFactor, context.autoCloseCDP, params.timestamp);
    // Create updated CDP with reduced debt
    const updatedCDP = createUpdatedCDP(params.cdp, allocation.principalPayment, allocation.feesPayment, newState, params.timestamp);
 ...
 * ```
 */
export const burnNYXUSDBatch = (burns, context) => {
    const results = [];
 ...
 * ```
 */
export const estimateMinBurnForHealthFactor = (cdp, targetHealthFactor, collateralPrice, stabilityFeeRate, timeElapsed) => {
    // Calculate total debt including accrued fees
 ...
 * ```
 */
export const calculateHealthFactorImprovement = (cdp, burnAmount, collateralPrice, stabilityFeeRate, timeElapsed) => {
    ...
    const afterBurn = calculateHealthFactorAfterBurn(cdp.collateralAmount, cdp.debtAmount, allocation.principalPayment, collateralPrice, cdp.config.liquidationRatio);
 ...
 * ```
 */
export const calculateFullClosureAmount = (cdp, stabilityFeeRate, timeElapsed) => {
    const accruedFees = calculateAccruedFees(cdp.debtAmount, stabilityFeeRate, timeElapsed);
