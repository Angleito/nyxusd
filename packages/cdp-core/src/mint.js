 ...
 * ```
 */
export const validateMintNYXUSD = (params, context) => {
 ...
 * ```
 */
export const calculateMaxMintableAmount = (cdp, collateralPrice, stabilityFeeRate, timeElapsed) => {
    // Calculate accrued fees
 ...
 * ```
 */
export const calculateHealthFactorAfterMint = (collateralAmount, currentDebt, mintAmount, accruedFees, collateralPrice, liquidationRatio) => {
 ...
 * ```
 */
export const updateCDPStateAfterMint = (_currentState, newHealthFactor, _liquidationRatio) => {
 ...
 * ```
 */
export const createUpdatedCDP = (cdp, mintAmount, accruedFees, newState, timestamp) => {
    return {
        ...cdp,
 ...
 * ```
 */
export const mintNYXUSD = (params, context) => {
    ...
    const newHealthFactor = calculateHealthFactorAfterMint(params.cdp.collateralAmount, params.cdp.debtAmount, params.mintAmount, accruedFees, context.collateralPrice, params.cdp.config.liquidationRatio);
    // Update CDP state based on new health factor
    const newState = updateCDPStateAfterMint(params.cdp.state, newHealthFactor, params.cdp.config.liquidationRatio);
    // Create updated CDP with increased debt
    const updatedCDP = createUpdatedCDP(params.cdp, params.mintAmount, accruedFees, newState, params.timestamp);
 ...
 * ```
 */
export const mintNYXUSDBatch = (mints, context) => {
    const results = [];
 ...
 * ```
 */
export const estimateAnnualBorrowingCost = (debtAmount, stabilityFeeRate) => {
 ...
 * ```
 */
export const calculateCollateralizationRatioAfterMint = (cdp, mintAmount, collateralPrice, stabilityFeeRate, timeElapsed) => {
    const accruedFees = calculateAccruedFees(cdp.debtAmount, stabilityFeeRate, timeElapsed);
