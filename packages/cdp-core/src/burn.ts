...
export const calculateBurnAllocation = (
  ...
  if (accruedFees > 0n && remainingBurn > 0n) {
    feesPayment = mkAmount(
      remainingBurn >= accruedFees ? accruedFees : remainingBurn,
    );
  ...
  if (currentDebt > 0n && remainingBurn > 0n) {
    principalPayment = mkAmount(
      remainingBurn >= currentDebt ? currentDebt : remainingBurn,
    );
  }
...
export const estimateMinBurnForHealthFactor = (
  ...
  const liquidationThreshold =
    (collateralValue * BigInt(10000)) / BigInt(cdp.config.liquidationRatio);
  const requiredDebt =
    (liquidationThreshold / BigInt(Math.floor(targetHealthFactor * 100))) *
    BigInt(100);
