...
import { mkCDPId, mkAmount, mkCollateralizationRatio, } from "./types";
/**
 ...
 * ```
 */
export const validateCDPCreation = (params, context) => {
 ...
 * ```
 */
export const calculateHealthFactor = (collateralAmount, debtAmount, collateralPrice, liquidationRatio) => {
    if (debtAmount === 0n) {
 ...
 * ```
 */
export const generateCDPId = (owner, collateralType, timestamp) => {
 ...
 * ```
 */
export const createInitialCDPState = (healthFactor, _liquidationRatio) => {
 ...
 * ```
 */
export const createCDP = (params, context) => {
    // Validate creation parameters
    const validationResult = validateCDPCreation(params, context);
    if (validationResult.isErr()) {
        return Result.err(validationResult.value);
    }
    // Generate unique CDP ID
    const cdpId = generateCDPId(params.owner, params.collateralType, context.currentTime);
    // Calculate health factor
    const healthFactor = calculateHealthFactor(params.collateralAmount, params.debtAmount, context.collateralPrice, params.config.liquidationRatio);
    // Create initial state
    const initialState = createInitialCDPState(healthFactor, params.config.liquidationRatio);
 ...
 * ```
 */
export const createCDPBatch = (requests, context) => {
    const cdps = [];
 ...
 * ```
 */
export const estimateMaxDebt = (collateralAmount, collateralPrice, minCollateralizationRatio) => {
    const collateralValue = (collateralAmount * collateralPrice) / BigInt(10 ** 18);
    const maxDebt = (collateralValue * BigInt(10000)) / BigInt(minCollateralizationRatio);
    return mkAmount(maxDebt);
};
/**
 ...
 * ```
 */
export const estimateMinCollateral = (debtAmount, collateralPrice, minCollateralizationRatio) => {
