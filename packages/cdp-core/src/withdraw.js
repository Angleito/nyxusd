...
import { mkAmount, mkCollateralizationRatio, } from "./types";
/**
 ...
 * ```
 */
export const validateWithdrawCollateral = (params, context) => {
 ...
 * ```
 */
export const calculateMaxWithdrawableAmount = (cdp, collateralPrice, safetyBuffer) => {
 ...
 * ```
 */
export const calculateHealthFactorAfterWithdraw = (currentCollateral, withdrawAmount, debtAmount, collateralPrice, liquidationRatio) => {
    if (debtAmount === 0n) {
 ...
 * ```
 */
export const updateCDPStateAfterWithdraw = (_currentState, newHealthFactor, _liquidationRatio) => {
 ...
 * ```
 */
export const createUpdatedCDP = (cdp, withdrawAmount, newState, timestamp) => {
    return {
        ...cdp,
 ...
 * ```
 */
export const withdrawCollateral = (params, context) => {
    ...
    const newHealthFactor = calculateHealthFactorAfterWithdraw(params.cdp.collateralAmount, params.withdrawAmount, params.cdp.debtAmount, context.collateralPrice, params.cdp.config.liquidationRatio);
    // Update CDP state based on new health factor
    const newState = updateCDPStateAfterWithdraw(params.cdp.state, newHealthFactor, params.cdp.config.liquidationRatio);
    // Create updated CDP with reduced collateral
    const updatedCDP = createUpdatedCDP(params.cdp, params.withdrawAmount, newState, params.timestamp);
    // Calculate remaining available collateral for future withdrawals
    const remainingAvailableCollateral = calculateMaxWithdrawableAmount(updatedCDP, context.collateralPrice, context.safetyBuffer);
 ...
 * ```
 */
export const withdrawCollateralBatch = (withdrawals, context) => {
    const results = [];
 ...
 * ```
 */
export const calculateHealthFactorImpact = (cdp, withdrawAmount, collateralPrice) => {
    const current = calculateCurrentHealthFactor(cdp, collateralPrice);
    const afterWithdraw = calculateHealthFactorAfterWithdraw(cdp.collateralAmount, withdrawAmount, cdp.debtAmount, collateralPrice, cdp.config.liquidationRatio);
 ...
 * ```
 */
export const estimateFreedCollateralValue = (withdrawAmount, collateralPrice) => {
