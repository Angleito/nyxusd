 ...
 * ```
 */
export const validateDepositCollateral = (params, context) => {
 ...
 * ```
 */
export const calculateHealthFactorAfterDeposit = (currentCollateral, depositAmount, debtAmount, collateralPrice, liquidationRatio) => {
    if (debtAmount === 0n) {
 ...
 * ```
 */
export const updateCDPStateAfterDeposit = (currentState, newHealthFactor, _liquidationRatio) => {
 ...
 * ```
 */
export const createUpdatedCDP = (cdp, depositAmount, newState, timestamp) => {
    return {
        ...cdp,
        collateralAmount: mkAmount(cdp.collateralAmount + depositAmount),
        state: newState,
        updatedAt: timestamp,
    };
};
 ...
 * ```
 */
export const depositCollateral = (params, context) => {
    // Validate deposit parameters and system state
    const validationResult = validateDepositCollateral(params, context);
    ...
    const newHealthFactor = calculateHealthFactorAfterDeposit(params.cdp.collateralAmount, params.depositAmount, params.cdp.debtAmount, context.collateralPrice, params.cdp.config.liquidationRatio);
    // Update CDP state based on new health factor
    const newState = updateCDPStateAfterDeposit(params.cdp.state, newHealthFactor, params.cdp.config.liquidationRatio);
    // Create updated CDP with additional collateral
    const updatedCDP = createUpdatedCDP(params.cdp, params.depositAmount, newState, params.timestamp);
    ...
    return Result.ok(depositResult);
};
/**
 ...
 * ```
 */
export const depositCollateralBatch = (deposits, context) => {
    const results = [];
 ...
 * ```
 */
export const calculateHealthFactorImprovement = (cdp, depositAmount, collateralPrice) => {
    const current = calculateCurrentHealthFactor(cdp, collateralPrice);
    const afterDeposit = calculateHealthFactorAfterDeposit(cdp.collateralAmount, depositAmount, cdp.debtAmount, collateralPrice, cdp.config.liquidationRatio);
 ...
 * ```
 */
export const estimateMinDepositForHealthFactor = (cdp, targetHealthFactor, collateralPrice) => {
    if (cdp.debtAmount === 0n) {
        return mkAmount(0n); // No debt means no deposit needed
    }
    ...
    return mkAmount(additionalCollateral);
};
