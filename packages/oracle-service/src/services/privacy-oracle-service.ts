...
class MockZKCircuit {
  ...
  async verifyProof(
    ...
    try {
      ...
      const isValid =
        proof.proof.length === 64 && // SHA256 hex length
...
export class PrivacyOracleService implements IPrivacyOracleService {
  ...
  readonly fetchPrivatePrice = async (
    ...
    try {
      ...
      const commitmentResult = await this.generatePriceCommitment(
        mockPrice,
        query.nonce,
      );
      ...
      const response: PrivateOracleResponse = {
        ...
        metadata: {
          ...
          encryptionStatus: query.privacyParams.encryptResponse
            ? "encrypted"
            : "plaintext",
        },
      };
      ...
      this.logAuditEntry({
        ...
        feedId: query.feedId,
        requesterHash: crypto
          .createHash("sha256")
          .update(query.requesterPublicKey)
          .digest("hex"),
        privacyLevel: this.config.privacyLevel,
        success: true,
  ...
  readonly verifyZKProof = async (
    ...
    try {
      ...
      const verificationResult = await this.zkCircuit.verifyProof(
        proof,
        publicInputs,
      );
      ...
      this.logAuditEntry({
        ...
        privacyLevel: this.config.privacyLevel,
        success:
          verificationResult._tag === "Right" && verificationResult.right,
