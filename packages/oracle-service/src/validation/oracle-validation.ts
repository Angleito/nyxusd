...
export const ExtendedOraclePriceValidationSchema = OraclePriceDataSchema.extend(
  {
    feedId: z
      ...
      .regex(
        /^[A-Z]{2,10}-[A-Z]{2,10}$/,
        "Feed ID must be in format ASSET-QUOTE",
      ),
    price: z
      .bigint()
      .positive()
...
export const ExtendedChainlinkRoundDataSchema = ChainlinkRoundDataSchema.extend(
  {
    answer: z
      .bigint()
      .positive()
      .refine((val) => val > 0, "Price answer must be positive"),
    updatedAt: z.bigint().refine((val) => {
...
export const ValidatedMultiOracleDataSchema = MultiOracleDataSchema.extend({
  responses: z
    ...
    .min(1)
    .max(10), // Support 1-10 oracle providers
}).refine(
...
export const createPriceRangeValidator =
  (minPrice: bigint, maxPrice: bigint, feedId: string) =>
...
export const createDeviationValidator =
  (referencePrice: bigint, maxDeviationPercent: number) =>
  (price: bigint): Either<ValidationError, bigint> => {
    const deviation =
      Number(
...
export const validatePriceData = (
  ...
  if (rules) {
    ...
    if (rules.referencePrice && rules.maxDeviation) {
      const deviation =
        Number(
          priceData.price > rules.referencePrice
            ? ((priceData.price - rules.referencePrice) * BigInt(10000)) /
                rules.referencePrice
            : ((rules.referencePrice - priceData.price) * BigInt(10000)) /
                rules.referencePrice,
        ) / 100;
 ...
 */
export const validateOracleQuery = (
  query: unknown,
): Either<ValidationError[], OracleQuery> => {
 ...
 */
export const validateChainlinkRoundData = (
  data: unknown,
): Either<ValidationError[], ChainlinkRoundData> => {
 ...
 */
export const validateMultiOracleData = (
  data: unknown,
): Either<ValidationError[], MultiOracleData> => {
 ...
 */
export const validateAggregationResult = (
  data: unknown,
): Either<ValidationError[], AggregationResult> => {
...
export const composeValidators =
  <T>(...validators: Array<(value: T) => Either<ValidationError, T>>) =>
...
export const validateArray =
  <T>(validator: (item: unknown) => Either<ValidationError[], T>) =>
...
export const getValidationPreset = (
  feedId: string,
): Option<PriceValidationRules> => {
  const preset =
    ASSET_VALIDATION_PRESETS[feedId as keyof typeof ASSET_VALIDATION_PRESETS];
