...
import { ChainlinkFeedAddresses, ChainlinkNetworkConfig } from '../types/chainlink-types';
 ...
 */
export const getFeedAddress = (network: string, feedId: string): string | undefined => {
  return CHAINLINK_FEED_ADDRESSES[network as keyof ChainlinkFeedAddresses]?.[feedId];
};
...
export const getSupportedFeeds = (network: string): string[] => {
  const feeds = CHAINLINK_FEED_ADDRESSES[network as keyof ChainlinkFeedAddresses];
 ...
 */
export const getOracleConfig = (category: keyof typeof DEFAULT_ORACLE_CONFIGS) => {
