...
class TokenService {
  ...
  private baseApiUrl = import.meta.env.VITE_API_URL 
  ...
  private async fetchCoinGeckoTokens(limit: number): Promise<ReadonlyArray<TokenInfo>> {
    ...
    if (data.success && Array.isArray(tokens)) {
      return tokens
        ...
        .filter((token): token is TokenInfo & { address: string; logoURI: string; decimals: number; tags: ReadonlyArray<string> } => Boolean(token.address) && Boolean(token.tags) && Boolean(token.logoURI) && Boolean(token.decimals)); // Filter out tokens without required fields
    }
