...
export function calculateRiskScore(walletData: WalletData): number {
  ...
  const maxConcentration = Math.max(
    ...assets.map((a) => (a.valueUSD / totalValue) * 100),
  );
...
export function identifyOpportunities(
  ...
  if (totalValue > 10000) {
    opportunities.push({
      ...
      potentialBenefit:
        "Potentially save 1-2% annually through tax optimization",
      priority: "low",
    });
  }
...
function generateRiskRecommendations(
  ...
  if (assetPercentages.length === 1) {
    recommendations.push(
      "Single asset portfolios carry significant risk. Diversify to protect your wealth",
    );
  }
