  ...
  conservative: [
    {
      ...
      riskLevel: "low",
      explanationTemplate:
        "This strategy focuses on providing liquidity to stable pairs, minimizing impermanent loss while generating consistent yields.",
    ...
    },
    {
      ...
      riskLevel: "low",
      explanationTemplate:
        "Staking nyxUSD provides a reliable income stream with minimal risk, backed by overcollateralized positions.",
      actionType: "stake",
      asset: "nyxUSD",
    },
    {
      ...
      riskLevel: "medium",
      explanationTemplate:
        "A modest allocation to CLMM provides upside potential while maintaining overall portfolio stability.",
      actionType: "allocate",
      asset: "CLMM",
    },
    {
      ...
      riskLevel: "low",
      explanationTemplate:
        "Keeping some funds liquid allows you to take advantage of market opportunities without disrupting your core positions.",
      actionType: "allocate",
      asset: "USDC",
    },
  ],
  moderate: [
    {
      ...
      riskLevel: "medium",
      explanationTemplate:
        "This balanced approach to liquidity provision offers good yields with moderate impermanent loss risk.",
      actionType: "provide-liquidity",
      asset: "nyxUSD/USDC",
    },
    {
      ...
      riskLevel: "medium",
      explanationTemplate:
        "Staking CLMM not only provides yield but also governance rights in the protocol.",
      actionType: "stake",
      asset: "CLMM",
    },
    {
      ...
      riskLevel: "medium",
      explanationTemplate:
        "Using CDPs with safe collateral ratios allows you to maintain exposure while accessing liquidity.",
      actionType: "open-cdp",
      asset: "ETH",
    },
    {
      ...
      riskLevel: "medium",
      explanationTemplate:
        "Diversification across multiple assets helps balance risk and return in your portfolio.",
      actionType: "allocate",
      asset: "Mixed",
    },
  ],
  aggressive: [
    {
      ...
      riskLevel: "high",
      explanationTemplate:
        "Aggressive CLMM strategies can generate significant returns but come with higher volatility.",
      actionType: "allocate",
      asset: "CLMM",
    },
    {
      ...
      riskLevel: "high",
      explanationTemplate:
        "Leveraging through CDPs can amplify returns but requires active management to avoid liquidation.",
      actionType: "open-cdp",
      asset: "ETH",
    },
    {
      ...
      riskLevel: "high",
      explanationTemplate:
        "High-volume volatile pairs offer exceptional fees but significant impermanent loss risk.",
      actionType: "provide-liquidity",
      asset: "CLMM/ETH",
    },
    {
      ...
      riskLevel: "high",
      explanationTemplate:
        "Keeping funds available for opportunistic trades allows you to capitalize on market inefficiencies.",
      actionType: "allocate",
      asset: "USDC",
    },
  ],
...
const normalizeAllocations = (
  ...
  let normalizedRecommendations = recommendations.map((rec) => ({
    ...
    allocationPercentage: Math.round(
      rec.allocationPercentage * normalizationFactor,
    ),
  }));

  // Handle rounding errors
  const normalizedTotal = normalizedRecommendations.reduce(
    (sum, rec) => sum + rec.allocationPercentage,
    0,
  );
  if (normalizedTotal !== 100) {
    const diff = 100 - normalizedTotal;
    const largestAllocation = normalizedRecommendations.reduce(
      (prev, current) =>
        current.allocationPercentage > prev.allocationPercentage
          ? current
          : prev,
    );
...
export const generateRecommendations = (
  ...
  try {
    ...
    const recommendations: Recommendation[] = templates.map((template) => {
      const adjustedAllocation = adjustAllocationForProfile(
        template,
        userProfile,
        walletData,
      );
      const allocationValue =
        (walletData.totalValueUSD * adjustedAllocation) / 100;
    ...
    const nonZeroRecommendations = recommendations.filter(
      (rec) => rec.allocationPercentage > 0,
    );

    // Normalize allocations to ensure they sum to 100%
    const normalizedRecommendations = normalizeAllocations(
...
export const calculateExpectedPortfolioReturn = (
  recommendations: Recommendation[],
): {
  min: number;
  max: number;
