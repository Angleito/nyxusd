...
const randomChoice = <T>(choices: T[]): T => {
  ...
  return choices[index]!; // Safe because we know choices has at least one element
};
...
export const getWalletAnalysisMessage = (walletData: WalletData): string => {
  const { totalValueUSD, tokenCount, mainHoldings, hasDefiPositions } =
    walletData;
...
// Re-export functions from naturalLanguageProcessor for backward compatibility
export {
  generateClarificationMessage,
  generateConfirmationMessage,
} from "../ai-assistant/naturalLanguageProcessor";
