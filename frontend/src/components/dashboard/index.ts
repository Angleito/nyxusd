// Dashboard Components Exports
export { ModernDashboard } from './ModernDashboard'
export { DashboardWithHero } from './DashboardWithHero'
export { default as HeroSection } from './HeroSection'
export { StatsCard } from './StatsCard'
export { WelcomeCard } from './WelcomeCard'
export { SystemHealthCard } from './SystemHealthCard'
export { OraclePricesCard } from './OraclePricesCard'
export { RecentActivityCard } from './RecentActivityCard'

// Types
export type { StatsCardProps } from './StatsCard'
export type { WelcomeCardProps } from './WelcomeCard'
export type { SystemHealthCardProps } from './SystemHealthCard'
export type { OraclePricesCardProps } from './OraclePricesCard'
export type { RecentActivityCardProps, Activity } from './RecentActivityCard'

// Default export - ModernDashboard for the new modern redesign
export { ModernDashboard as default } from './ModernDashboard'