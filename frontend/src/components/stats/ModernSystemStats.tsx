...
export const ModernSystemStats: React.FC = () => {
  ...
  const { data: cdpData, isLoading: cdpsLoading } = useQuery({
    queryKey: ["cdps"],
    queryFn: fetchCDPs,
  });
  ...
  const getRiskDistribution = () => {
    ...
    const safe = (cdpData as any).cdps.filter(
      (cdp: any) => cdp.healthFactor >= 2,
    ).length;
    const warning = (cdpData as any).cdps.filter(
      (cdp: any) => cdp.healthFactor >= 1.5 && cdp.healthFactor < 2,
    ).length;
    const danger = (cdpData as any).cdps.filter(
      (cdp: any) => cdp.healthFactor < 1.5,
    ).length;

    return [
      { name: "Safe CDPs (≥2.0x)", value: safe, color: COLORS.safe },
      {
        name: "Warning CDPs (1.5-2.0x)",
        value: warning,
  ...
  const getTVLTrends = () => {
    const currentTVL = (systemStats as any)?.totalCollateral
      ? parseInt((systemStats as any).totalCollateral) / 1e18
      : 0;
    const periods = [
      "30d ago",
      "25d ago",
    ...
    return periods.map((period, index) => ({
      period,
      tvl:
        currentTVL * (0.7 + index * 0.05) +
        (Math.random() - 0.5) * currentTVL * 0.1,
      supply: (systemStats as any)?.totalDebt
        ? (parseInt((systemStats as any).totalDebt) / 1e18) *
          (0.6 + index * 0.06)
        : 0,
    }));
  };
  ...
  const totalTVL = (systemStats as any)?.totalCollateral
    ? parseInt((systemStats as any).totalCollateral) / 1e18
    : 0;
  const totalSupply = (systemStats as any)?.totalDebt
    ? parseInt((systemStats as any).totalDebt) / 1e18
    : 0;
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-500">
                    {(
                      (item.value /
                        riskData.reduce((sum, item) => sum + item.value, 0)) *
                      100
                    ).toFixed(1)}
                    %
                  </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="text-center group">
            ...
            <p className="text-gray-600 leading-relaxed">
              Collateral amounts and debt positions are shielded using
          ...
          <div className="text-center group">
            ...
            <p className="text-gray-600 leading-relaxed">
              Liquidation process maintains privacy while ensuring system
              stability
            </p>
            <div className="mt-4 text-sm">
          ...
          <div className="text-center group">
            ...
            <p className="text-gray-600 leading-relaxed">
              Powered by Midnight&apos;s DUST token for privacy-preserving
              transaction fees
            </p>
            <div className="mt-4 text-sm">
