...
const ProgressBar: React.FC<ProgressBarProps> = ({ value, max, label, color = 'blue' }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`h-2 ${backgroundClasses[color]} rounded-full overflow-hidden`}>
        <div 
...
export const MetricsGrid: React.FC<MetricsGridProps> = ({
  ...
  const systemHealth = collateralizationRatio >= 200 ? 'excellent' : 
                      collateralizationRatio >= 150 ? 'good' : 
                      collateralizationRatio >= 120 ? 'caution' : 'critical'
      ...
      <div className="card group hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className={`font-semibold capitalize ${getHealthColor(systemHealth)}`}>
              {systemHealth}
            </span>
          </div>
          <ProgressBar 
            ...
            max={300} 
            label="Safety Ratio" 
            color={systemHealth === 'excellent' ? 'green' : 
                   systemHealth === 'good' ? 'blue' :
                   systemHealth === 'caution' ? 'yellow' : 'red'}
          />
        </div>
      </div>
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">
              {systemStats?.stabilityFeeRate ? (systemStats.stabilityFeeRate / 100).toFixed(1) : '0.0'}% APR
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">Emergency Shutdown</span>
            <span className={`font-medium ${systemStats?.emergencyShutdown ? 'text-red-600' : 'text-green-600'}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold">
                {totalTVL > 0 ? ((totalTVL - totalSupply) / totalTVL * 100).toFixed(1) : '0'}%
              </span>
            </div>
            <ProgressBar 
              value={totalTVL > 0 ? ((totalTVL - totalSupply) / totalTVL * 100) : 0} 
              max={100} 
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div className="space-y-2">
            ...
            <div className="text-sm font-medium">
              <AnimatedCounter value={Math.floor(Math.random() * 1000) + 500} suffix=" TPS" />
            </div>
